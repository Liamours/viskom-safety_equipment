...
Wang et al.~\cite{ref5}
 & 2021 & YOLOv5 & Construction & GDUTâ€‘Hardhat (custom) & None & No & -- & No & mAP 0.866 (YOLOv5x) \\
...
Vukicevic et al.~\cite{vukicevic2022compliance}
 & 2022 & YOLOv5 & Industrial & Custom headâ€‘mounted images & ROI & Yes & Binary & No & Precision 0.920 / Recall 0.611 \\
Ahmed et al.~\cite{ref23}
 & 2023 & Faster Râ€‘CNN & General & Mixed public + webâ€‘scraped & None & Single & -- & No & mAP 0.960 \\
...
Imam et al.~\cite{ref12}
 & 2024 & YOLOv8-pose + YOLOv8l & Mining & Selfâ€‘collected underground mine & Filter & Yes & Binary & No & Pose mAP 0.90 / PPE mAP 0.97 \\
...
\subsubsection{IoU-Based Matching} Each PPE detection is assigned to the person whose anatomical body region yields the highest IoU. An assignment is accepted if $IoU > \tau_a = 0.10$. This permissive threshold is intentionally lower than the standard 0.50 because overlap is computed against compact anatomical anchors rather than identical object boxes. Since wearable items (e.g., helmets, gloves) occupy only a fraction of their corresponding body regions, a higher threshold would reject valid assignments. In multi-person scenes, each item is attributed to at most one worker; if multiple detections of the same class match one person, only the highest-confidence instance is retained. 
...
\begin{table}[H]
    \centering
    \caption{Mean Pipeline Latency Across All Test Sets}
    \label{tab:runtime}
    \setlength{\tabcolsep}{4pt}
    \footnotesize
    \begin{tabular}{lrrrr}
        \hline
        \textbf{Dataset} & \textbf{PPE det.} & \textbf{Pose det.} & \textbf{Anchor+C} & \textbf{Total} \\
                         & (ms)              & (ms)               & (ms)              & (ms) / FPS     \\
        \hline
        CPPE-5 ($n=29$)      & 29.80 & 43.59 & 0.41 & 73.93 / 13.53 \\
        CHV ($n=133$)        & 33.28 & 46.64 & 0.79 & 80.86 / 12.37 \\
        SH17 ($n=1{,}620$)   & 29.12 & 62.19 & 0.39 & 91.83 / 10.89 \\
        \hline
        \textbf{Overall} ($n=1{,}782$) & \textbf{29.44} & \textbf{60.73} & \textbf{0.42} & \textbf{90.72 / 11.02} \\
        \hline
        \multicolumn{5}{l}{\footnotesize Report gen.: 0.13~ms ($<$1\%). GPU: NVIDIA RTX~4050 (6~GB).} \\
        \multicolumn{5}{l}{\footnotesize Anchor+C = anchor construction + compliance.} \\
    \end{tabular}
\end{table}


Table~\ref{tab:runtime} reports mean latency per pipeline stage measured across all three
test sets ($n = 1{,}782$ images), with each stage timed independently. The dominant
bottleneck is pose detection (YOLOv8x-pose) at 60.73~ms (67\% of total), followed by
PPE detection (YOLO26L) at 29.44~ms (32\%). Anchor construction and compliance
evaluation account for only 0.42~ms ($<$1\%), confirming that the assignment logic
imposes negligible overhead relative to neural network inference. The mean end-to-end
latency of 90.72~ms corresponds to 11.02~FPS, below the 25~FPS threshold commonly
required for real-time video processing. The system is therefore suited for batch or
near-real-time inspection scenarios rather than continuous video monitoring.


\subsection{Ablation Study}
We perform a sensitivity analysis on the PPE-to-person assignment module by sweeping $\tau_a$ and $\delta$ across 20 configurations. Table VII presents the assignment accuracy for both CHV and SH17 datasets.

\begin{table}[!htbp]
    \centering
    \caption{Assignment Accuracy Ablation: $\tau_a \times \delta$ Grid ($\tau_v = 0.30$ Fixed)}
    \label{tab:ablation}
    \small 
    \begin{tabular}{lcccc} % 1 label + 4 data kolom
        \toprule 
        \multicolumn{5}{c}{\textbf{CHV} (94 multi-person images)} \\ \midrule 
        $\tau_a \backslash \delta$ & 0.10 & 0.15 & 0.20 & 0.25 \\ \midrule 
        0.10  & \textbf{0.9844} & 0.9820 & 0.9821 & 0.9795 \\
        0.15  & 0.9842 & 0.9844 & 0.9819 & 0.9793 \\
        0.20  & 0.9839 & 0.9840 & 0.9843 & 0.9791 \\ \midrule \midrule 
        \multicolumn{5}{c}{\textbf{SH17} (582 multi-person images)} \\ \midrule
        $\tau_a \backslash \delta$ & 0.10 & 0.15 & 0.20 & 0.25 \\ \midrule
        0.10 & \textbf{0.9723} & 0.9734 & 0.9632 & 0.9614 \\
        0.15 & 0.9598 & 0.9668 & 0.9692 & 0.9617 \\
        0.20 & 0.9557 & 0.9634 & 0.9636 & 0.9659 \\ \bottomrule 
        \noalign{\smallskip}
        \multicolumn{5}{p{6.49cm}}{\footnotesize
        Default: $\tau_a = 0.10, \delta = 0.10$ (bold). 
        CHV: 0.977--0.987. 
        SH17: 0.946--0.974.
        }
    \end{tabular}
...
Table~\ref{tab:ablation} demonstrates PACT's robustness to parameter variations. On CHV, 
an accuracy spread below 0.01 reflects stable assignment under clear geometric separation 
between workers. On the denser SH17, accuracy remains above 0.946 despite increasing $\delta$, 
suggesting that the default parameters ($\tau_a=0.10, \delta=0.10$) generalize well across domains 
without intensive tuning.

\subsection{Limitations}
Several limitations exist in the proposed PACT pipeline. Detection errors in Stage 1 directly propagate to the report, and the assignment module remains susceptible to misattribution in dense scenes. Furthermore, PACT’s generalizability to medical domains is partially unverified due to the lack of person-class annotations in CPPE-5. Additionally, following standard PPE benchmarks [5]–[7], results are reported as point estimates; however, we recognize that the limited size of the CPPE-5 test set may introduce variance in single-point mAP estimates. Future work will address these gaps by performing manual annotation of a CPPE-5 subset for healthcare validation, incorporating statistical variance reporting, and developing temporal tracking to maintain compliance states.
