"""
Ablation: assignment accuracy vs tau_a and delta on CHV and SH17.
tau_v fixed at 0.30. Sweeps tau_a x delta on multi-person frames only.
...
TAU_V       = 0.30
IOU_MATCH   = 0.50
OUT         = ROOT / "results/eval/ablation_assignment.csv"
...
def load_gt(label_path, iw, ih, classes, person_ids):
    ...
    for line in label_path.read_text().splitlines():
        ...
        (persons if cid in person_ids else ppe).append(entry)
    return persons, ppe
...
def compute_accuracy(cache, tau_a, delta):
    ...
    for entry in cache:
        ...
        if len(gt_persons) < 2 or len(persons) < 2:
            continue
...
def main():
    ...
    for ds_name, cfg in DATASETS.items():
        ...
        print(f"  {len(cache)} multi-person images")
